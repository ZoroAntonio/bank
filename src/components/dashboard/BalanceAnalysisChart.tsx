      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        {hoveredIndex !== null ? (
          ...
          <>
            ...
            <span className="text-lg font-bold text-slate-900">{formatUsd(total, locale)}</span>
          </>
        )}
      </div>
    </div>
...
function WaveGraph({ items, showBalances, locale }: { items: ColumnItem[]; showBalances: boolean; locale: string }) {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
          ...
          return (
            <g key={tick}>
              ...
              <text x={padding.left - 10} y={y + 4} textAnchor="end" className="text-[10px]" fill="#94a3b8">
                {showBalances ? formatCompact(tick, locale) : ''}
              </text>
            </g>
          );
        })}
...
export default function BalanceAnalysisChart({
  ...
  const fiatBarItems = fiatBalances.map((fb) => ({
    ...
    value: fb.balance * (APPROX_USD_RATES[fb.currency] || 1),
    color: FIAT_COLORS[fb.currency] || '#64748b',
    displayValue: new Intl.NumberFormat(currentLocale, {
      style: 'currency',
  ...
  const cryptoBarItems = cryptoBalances
    ...
    .map((c) => ({
      ...
      value: c.balance * (APPROX_CRYPTO_USD[c.symbol] || 1),
      color: CRYPTO_COLORS[c.symbol] || '#94a3b8',
      displayValue: `${c.balance.toLocaleString(currentLocale, { maximumFractionDigits: 4 })} ${c.symbol}`,
    }));
      ...
      <div className="p-6">
        ...
        {viewMode === 'bar' && (
          <div className="space-y-4">
            <WaveGraph items={filteredBarItems} showBalances={showBalances} locale={currentLocale} />
