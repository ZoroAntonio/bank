...
const fiatCurrencies = [
  { key: 'usd', flag: 'https://flagcdn.com/w80/us.png', flagKey: 'us', code: 'USD' },
  { key: 'eur', flag: 'https://flagcdn.com/w80/eu.png', flagKey: 'eu', code: 'EUR' },
  { key: 'cad', flag: 'https://flagcdn.com/w80/ca.png', flagKey: 'ca', code: 'CAD' },
];

const cryptoCurrencies = [
  { key: 'bitcoin', code: 'BTC', logo: `${CRYPTO_CDN}/BTC.svg` },
  { key: 'ethereum', code: 'ETH', logo: `${CRYPTO_CDN}/ETH.svg` },
  { key: 'tether', code: 'USDT', logo: `${CRYPTO_CDN}/USDT.svg` },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="mb-14">
          <div className="flex items-center gap-4 mb-4">
            ...
            <span className="text-white text-xs font-semibold tracking-[0.2em] uppercase">{t('home.currencies.eyebrow')}</span>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-10">
          <div>
            <div className="flex items-center gap-3 mb-5">
              <span className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/60">{t('home.currencies.fiat')}</span>
              ...
              {fiatCurrencies.map((c) => (
                ...
                >
                  <div className="w-10 h-7 rounded overflow-hidden border border-white/20 group-hover:border-white/35 transition-colors flex-shrink-0">
                    <img src={c.flag} alt={t(`home.currencies.flag.${c.flagKey}`)} className="w-full h-full object-cover" />
                  </div>
          ...
          <div>
            <div className="flex items-center gap-3 mb-5">
              <span className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/60">{t('home.currencies.crypto')}</span>
              <div className="flex-1 h-px bg-white/15" />
            </div>
            <div className="space-y-3">
              {cryptoCurrencies.map((c) => (
                ...
                >
                  <div className="w-8 h-8 rounded-lg overflow-hidden flex-shrink-0 bg-white/90 p-1.5 flex items-center justify-center">
                    <img src={c.logo} alt={t(`home.currencies.${c.key}`)} className="w-full h-full object-contain" />
                  </div>
                  <div className="flex-1">
                    <div className="font-medium text-white text-sm">{t(`home.currencies.${c.key}`)}</div>
                  </div>
            ...
            <p className="mt-5 text-xs tracking-wide text-white/70">{t('home.currencies.more')}</p>
          </div>
        </div>
      </div>
    </section>
